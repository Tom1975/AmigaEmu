...
#ifdef LOG_MIXER
#define LOG(str)  if (log_) log_->Log (ILogger::Severity::SEV_DEBUG, str);
#define LOGEOL if (log_) log_->Log (ILogger::Severity::SEV_DEBUG, "\n");
#define LOGB(str)  if (log_) log_->Log (ILogger::Severity::SEV_DEBUG, "%2.2X", str);
#else
#define LOG(str)
#define LOGB(str)
#define LOGEOL
...
bool SoundMixer::GetNewSoundFile(char * buffer, unsigned int size)
{
   ...
   char exe_path[MAX_PATH];   // TODO : Handle this in a better way
   strcpy(exe_path, ".\\REC\\");
...
SoundSource::SoundSource(SoundMixer * sound_hub): sound_mixer_(sound_hub)
{
   ...
   {
      volume_[i] = ((1.0f / (pow(sqrt(2.0f), (0xFF- i))))) ;
   }
#endif
}
