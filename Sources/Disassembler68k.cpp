...
unsigned int Disassembler68k::BchgDnOpcode_(Motherboard* motherboard, unsigned short opcode, unsigned int pc, std::string& str_asm)
{
   std::stringstream sstream;
   std::string str_opcode;
   unsigned short data_register_numer = (opcode >> 9) & 0x7;

   sstream << "bchg D" << std::hex << std::uppercase << data_register_numer;
   pc += DisassembleAddressingMode(motherboard, pc, (opcode >> 3) & 0x7, (opcode) & 0x7, 3, str_opcode);
   sstream << ", " << str_opcode;
   str_asm = sstream.str();
